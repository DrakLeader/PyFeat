...
Names = ["LR", "KNN", "DT", "NB", "Bagging", "RF", "AB", "GB", "SVM", "LDA"]
...
def runClassifiers(args):
    ...
    for classifier, name in zip(Classifiers, Names):
        ...
        for train_index, test_index in cv.split(X, y):
            ...
            avePrecision.append(
                average_precision_score(y_true=y_test, y_score=y_proba)
            )  # auPR
        ...
        plt.plot(
            ...
            mean_TPR,
            linestyle="-",
            label="{} ({:0.3f})".format(name, mean_auc),
            lw=2.0,
        )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "-cv", "--nFCV", type=int, help="Number of crossValidation", default=10
    )
    p.add_argument(
        "-data",
        "--dataset",
        type=str,
        help="~/dataset.csv",
        default="optimumDataset.csv",
    )
    p.add_argument(
        "-roc", "--auROC", type=int, help="Print ROC Curve", default=1, choices=[0, 1]
    )
    p.add_argument(
        "-box",
        "--boxPlot",
        type=int,
        help="Print Accuracy Box Plaot",
        default=1,
        choices=[0, 1],
    )
